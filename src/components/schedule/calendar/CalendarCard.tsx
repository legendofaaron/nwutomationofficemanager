...
const CalendarCard: React.FC<CalendarCardProps> = ({
  ...
  const renderDayWithTasks = useCallback((props: CalendarDayProps) => {
    ...
    return <DroppableArea id={`calendar-day-${format(date, 'yyyy-MM-dd')}`} acceptTypes={['task', 'employee', 'crew', 'client', 'booking', 'todo']} onDragEnter={e => {
    ...
    }} className={cn("relative flex flex-col items-center justify-center w-full h-full transition-colors duration-150", isBeingDraggedOver && isDragging && "bg-[#4A4A4A]/40 outline-dashed outline-2 outline-[#BBBBBB]")} activeClassName="outline-dashed outline-2 outline-[#BBBBBB]/50">
        <div className="flex flex-col items-center text-[#DDDDDD]">
          {children}
          {dayTasks.length > 0 && <div className="absolute bottom-1 flex justify-center space-x-0.5 mt-0.5">
              {dayTasks.length <= 3 ? dayTasks.map((_, idx) => <div key={idx} className="w-1 h-1 bg-[#BBBBBB] rounded-full" />) : <>
  ...
  return <Card className="bg-[#222222] border-[#444444]">
